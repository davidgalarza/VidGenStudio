...
Las conversaciones pueden reunir varios hablantes y oyentes en una escena narrativa. Las intervenciones separan las palabras de la interpretación y la acción; el sistema las reparte en tomas automáticas, compartidas o alternadas. Las referencias de escenarios vacíos se reutilizan para orientar la continuidad del lugar. El catálogo contiene 58 estilos distintos, con 36 opciones para voz en off y 32 para personajes hablando. Sus muestras son ilustraciones estáticas, no plantillas, previsualizaciones de los ajustes ni referencias adjuntadas automáticamente a Google. No se promete identidad de voz fija, pronunciación literal ni escenografía idéntica.
...
Historia empieza solo con las palabras que se escucharán. Un monólogo no necesita personajes configurados; una conversación identifica en el texto quién dice cada parte; en voz en off el texto se lee como narración literal. El modo y el estilo son selecciones visibles con valores iniciales de voz en off y Realista; no hay una opción automática de entrada. La dirección adicional es opcional. Crear propuesta pide a Gemini organizar el texto original y proponer escenas, reparto, voces, acciones, lugares y referencias visuales respetando la configuración elegida. La generación de referencias puede incluirse en esta etapa. La propuesta permanece editable antes de producir: se pueden revisar palabras e imágenes, reordenar, dividir o unir escenas, ajustar intervenciones y participantes, seleccionar o regenerar referencias y cambiar voz y ajustes de salida.

La galería visual permite buscar por técnica o uso, filtrar, marcar favoritos, ver detalles y comparar hasta tres estilos, incluidos los personalizados. Abrir una muestra no aplica la selección: la acción Usar es explícita. Cada modo muestra seis controles: cinco comunes de ritmo, cámara, luz, color y detalle, y explicación o actuación según corresponda. Los parámetros prevalecen sobre la base; las indicaciones personalizadas prevalecen sobre ambos dentro del ámbito visual, sin modificar el guion ni el modo. Las ediciones de nombre o parámetros se conservan al alternar modos. Mis estilos guarda perfiles y archivos en una biblioteca local separada; editar, clonar o eliminar una entrada no reescribe la configuración copiada en proyectos anteriores.

Los ajustes de estilo sin aplicar se pueden retomar después de cerrar el modal o cambiar entre Escenas y Voz y estilo. Permanecen solo en memoria mientras siga montada la vista: recargar o salir de ella los descarta. Cancelar descarta el borrador y Usar aplica y limpia el pendiente; guardar en Mis estilos persiste una copia y sus medios. La selección ya aplicada mantiene el guardado habitual del guion y el proyecto. Los favoritos son locales al navegador.

El análisis de estilo admite hasta seis imágenes o vídeos y 14 MiB combinados. Subirlos es local; Analizar con Gemini los envía mediante una solicitud explícita de pago con la clave personal. El resultado se revisa y aplica manualmente, y no reemplaza ajustes al fallar, cancelarse o llegar tarde. Los archivos no se adjuntan automáticamente al generar: las instrucciones y parámetros extraídos orientan planificación, referencias y vídeos. No se garantiza una reproducción exacta. La biblioteca y sus archivos no se sincronizan ni forman una copia de seguridad del proyecto.
