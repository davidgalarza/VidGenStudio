...
**The Script First Rule.** Give the script the main editable area on entry. Keep optional direction controls collapsed initially, with reference generation and its scope visible beside the proposal action. Use the three named stages—Guion, Propuesta, Producción—to explain the current state while retaining the return to the clip library.
...
Production uses the studio's existing clip previews and controls. Keep narration audition distinct from video regeneration, explain that regenerating a take preserves its voice-over, and identify consecutive parts that share narration. Show measured audio timing without word-by-word highlighting or precision the provider has not supplied. Preserve access to prior takes, downloadable materials and the editable montage.
